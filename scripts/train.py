...
from src.data_utils.data_utils import get_patient_ids_from_npy, load_brats21_volumes
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train script with optional resume from checkpoint")
...
if __name__ == "__main__":
    ...
    from src.data_utils.brats_dataset import build_monai_persistent_dataset, CacheLocalitySampler, _worker_init
    ...
    if args.checkpoint is not None and os.path.exists(args.checkpoint):
        ...
        # if ckpt is a dict that contains model/optimizer state info
    ...
    for epoch in range(start_epoch, NUM_EPOCHS):
        ...
        if len(val_dataloader) > 0:
            ...
            if avg_fg_dice > best_val_metric:
                ...
                # Save a full checkpoint (model + optimizer + scheduler + meta)
                ckpt_to_save = {
                    'epoch': epoch,
                ...
                torch.save(ckpt_to_save, "best_checkpoint.pth")
                # Backwards compatible: also write model weights only
