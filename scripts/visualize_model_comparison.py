...
PATIENT_FILTER = ['patient112', 'patient113', 'patient114']
...
MODEL_CONFIGS = {
    ...
    'MedNeXt': {
        'weights': PROJECT_ROOT / 'comparison' / 'MedNeXt' / 'MedNeXt_work_dir' / 'nnUNet_results' / 
                   'nnUNet' / '3d_fullres' / 'Task001_ACDC' / 
                   'nnUNetTrainerV2_MedNeXt_S_kernel3__nnUNetPlansv2.1_trgSp_1x1x1' / 'fold_0' / 'model_best.model',
        'type': 'mednext'
    },
    'nnUNet': {
...
def load_mednext(weights_path):
    """Load MedNeXt model (3D architecture).
    
    MedNeXt is a 3D segmentation model, so we load it and cache volume predictions.
    """
    mednext_path = PROJECT_ROOT / 'comparison' / 'MedNeXt'
    sys.path.insert(0, str(mednext_path))
    
    from nnunet_mednext.network_architecture.mednextv1.create_mednext_v1 import create_mednext_v1
    
    # Create MedNeXt Small model (same as training)
    model = create_mednext_v1(
        num_input_channels=1,
        num_classes=NUM_CLASSES,
        model_id='S',  # Small
        kernel_size=3,
        deep_supervision=True  # Was trained with DS
    ).to(DEVICE)
...
def load_model(model_name, config):
    ...
    elif config['type'] == 'mednext':
        return load_mednext(weights_path)
...
def inference_unetpp(model, image):
    ...
    if h != IMG_SIZE or w != IMG_SIZE:
        # Resize each channel
        image_resized = np.zeros((5, IMG_SIZE, IMG_SIZE), dtype=image.dtype)
        for i in range(5):
...
# Global cache for MedNeXt 3D predictions (volume_id -> 3D prediction array)
MEDNEXT_CACHE = {}

def inference_mednext(model, image, vol_id=None, slice_idx=None, volume_data=None):
    """Inference with MedNeXt (3D model).
    
    MedNeXt is a 3D model, so we need to run inference on the entire volume
    and then extract the slice prediction. Results are cached.
    """
    if vol_id is None:
        # Fallback: just return zeros if no volume info
        if image.ndim == 3:
    ...
    # Check cache
    if vol_id in MEDNEXT_CACHE:
        pred_vol = MEDNEXT_CACHE[vol_id]
        return pred_vol[:, :, slice_idx]
    
    # Need to run full volume inference
    if volume_data is None:
        # Cannot proceed without volume data
        if image.ndim == 3:
            h, w = image.shape[1], image.shape[2]
        else:
            h, w = image.shape
    ...
    vol_tensor = torch.from_numpy(vol).unsqueeze(0).unsqueeze(0).float().to(DEVICE)
    
    with torch.no_grad():
        out = model(vol_tensor)
        if isinstance(out, (list, tuple)):
            out = out[-1]  # Last output if deep supervision
        pred = torch.argmax(torch.softmax(out, dim=1), dim=1).squeeze(0)
    
    pred_vol = pred.cpu().numpy()  # (H, W, D)
    
    # Cache the result
    MEDNEXT_CACHE[vol_id] = pred_vol
    
    return pred_vol[:, :, slice_idx]


def inference(model, image, model_type, vol_id=None, slice_idx=None, volume_data=None):
    ...
    if model_type == 'pie_unet':
        ...
        return inference_mednext(model, image, vol_id, slice_idx, volume_data)
...
def load_test_data(num_cases=-1):
    ...
    # Filter by patient if PATIENT_FILTER is set
    if PATIENT_FILTER is not None:
        filtered_volume_ids = []
        for vol_id in test_volume_ids:
            patient_id = vol_id.split('_')[0]
            if patient_id in PATIENT_FILTER:
                filtered_volume_ids.append(vol_id)
        test_volume_ids = filtered_volume_ids
        print(f"Filtered to patients: {PATIENT_FILTER}")
    
    print(f"Found {len(test_volume_ids)} test volumes")
    ...
    if num_cases == -1 or num_cases >= total_slices:
        # Load all slices
        selected_indices = list(range(total_slices))
    else:
        # Select evenly distributed cases
    ...
    for idx in selected_indices:
        ...
        vol_path = test_dataset.volume_paths[vol_idx]
        
        # Extract patient ID (e.g., "patient101" from "patient101_ED")
        patient_id = vol_id.split('_')[0]
        ...
        cases.append({
            ...
            'vol_id': vol_id,
            'slice_idx': slice_idx,
            'vol_path': vol_path  # For MedNeXt 3D inference
        })
...
def main():
    ...
    for i, case_data in enumerate(cases):
        ...
        vol_path = case_data.get('vol_path', None)
        ...
        # Load full volume for MedNeXt (3D model) - cached by volume
        volume_data = None
        if vol_path and os.path.exists(vol_path):
            volume_data = np.load(vol_path)
        ...
        for model_name, model_info in models.items():
            ...
            # Run inference (pass volume info for MedNeXt)
            pred = inference(model_info['model'], image, model_info['type'],
                           vol_id=vol_id, slice_idx=slice_idx, volume_data=volume_data)
