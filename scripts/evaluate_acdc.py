...
def parse_args():
    ...
    parser.add_argument('--weights', type=str, default="weights/best_model_acdc_no_anatomical.pth", 
                        help="Path to model weights (default: weights/best_model_acdc_no_anatomical.pth)")
...
def main():
    ...
    print(f"Evaluating ACDC model on test set")

    test_transform = A.Compose([
        ToTensorV2(),
    ])

    print("\nLoading test data ACDC")

    PREPROCESSED_ROOT = os.path.join(config.PROJECT_ROOT, "preprocessed_data", "ACDC")
    ...
    print(f"Loading volume IDs from: {test_npy_dir}")
    test_volume_ids = get_acdc_volume_ids(test_npy_dir)
    print(f"Total test volumes: {len(test_volume_ids)}")
    ...
    for vol_id in tqdm(sorted_vol_ids, desc="Processing Volumes"):
        ...
        # Optional: Print info per case? Concise one-liner
        # d_avgs = [per_class_dice[c][-1] for c in range(1, NUM_CLASSES)]
        # print(f"Case {vol_id}: Dice {d_avgs}")
    ...
    results_file = "results/acdc_test_results_3d.txt"
    with open(results_file, 'w') as f:
        f.write("="*60 + "\n")
        f.write("ACDC TEST SET RESULTS (3D VOLUMETRIC)\n")
        f.write("="*60 + "\n\n")
