...
class MnMDataset25DOptimized(Dataset):
    def __init__(
        self, 
        npy_dir: str,
        ...
        print(f"MnMDataset25DOptimized: {len(self.index_map)} slices from {len(self.volume_paths)} volumes")
...
def get_mnm_volume_ids(npy_dir: str):
    metadata_path = os.path.join(npy_dir, 'metadata.json')
    if not os.path.exists(metadata_path):
        raise FileNotFoundError(f"metadata.json not found: {metadata_path}")
